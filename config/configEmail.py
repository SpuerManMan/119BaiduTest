...
class Email:
    ...
    def __init__(self):
        ...
        self.receiver = RC.get_Email('receiver')
        self.smtpserver = RC.get_Email('smtpserver')
        self.username = RC.get_Email('username')
        self.password = RC.get_Email('password')
        self.emflag = RC.get_Email('emflag')
        ...
        self.zip_file=os.path.join(self.log.proDir,'testFile','test.zip')
    # 设置邮件标题
    def set_header(self):
    ...
    def set_content(self):
        try:
            with open(os.path.join(self.retult_path,'report.html'),'rb')as f:
                content=f.read()
    ...
    def set_file(self):
         with zipfile.ZipFile(self.zip_file,'w',zipfile.ZIP_DEFLATED)as zipf:
             ...
             zipf.close()
         att1=MIMEText(open(self.zip_file,'rb').read(),'base64','utf-8')
         att1["Content-Type"] = 'application/octet-stream'
         att1["Content-Disposition"] = 'attachment; filename="test.zip"'
         self.message.attach(att1)

         att2 = MIMEText(open(os.path.join(self.retult_path,'report.html'), 'rb').read(), 'base64', 'utf-8')
